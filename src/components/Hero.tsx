...
const ecosystemContainerVariants: Variants = {
...
const ecosystemNodeVariants: Variants = {
...
// Living adaptive ecosystem variants for orbiting nodes with variable durations and path offsets
const livingOrbitVariants: Variants = {
  hidden: { opacity: 0, scale: 0.6 },
  visible: {
    opacity: 1,
    scale: 1,
  ...
  living: ({
    ...
    x: [0, xOffset, -xOffset * 0.75, xOffset * 0.45, 0],
    y: [0, yOffset, -yOffset * 0.75, yOffset * 0.45, 0],
    scale: [1, scaleShift, 0.98, scaleShift * 0.99, 1],
    rotate: [0, rotateShift, -rotateShift * 0.7, 0],
    transition: {
      duration,
      delay: pathOffset,
      repeat: Infinity,
...
// Nexus center breathing pulse variant
const livingNexusVariants: Variants = {
...
const telemetryCardVariants: Variants = {
      ...
      <div className="w-full px-6 lg:px-[10%] relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
          ...
          >
            {/* Eyebrow */}
            <motion.div
              variants={shouldReduceMotion ? undefined : headlineItemVariants}
              className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-950/50 border border-violet-700/30 text-violet-300 text-xs font-semibold uppercase tracking-wider mt-[10px] mb-6 shadow-inner"
            >
            ...
            <motion.h1
              variants={shouldReduceMotion ? undefined : headlineItemVariants}
              className="text-4xl sm:text-6xl xl:text-7xl font-extrabold tracking-tight text-white leading-[1.05] mb-6 font-display"
            >
              Your Business.{' '}
            ...
            <motion.p
              variants={shouldReduceMotion ? undefined : headlineItemVariants}
              className="text-lg sm:text-xl text-zinc-300 font-normal leading-relaxed max-w-2xl mb-4"
            >
            ...
            <motion.div
              variants={shouldReduceMotion ? undefined : headlineItemVariants}
              className="flex items-center gap-3 py-2 px-3.5 rounded-lg bg-white/[0.03] border border-white/[0.08] mb-8"
            >
            ...
            <motion.div
              variants={shouldReduceMotion ? undefined : headlineItemVariants}
              className="grid grid-cols-3 gap-6 pt-10 mt-10 border-t border-white/[0.08] w-full max-w-xl"
            >
              <div>
          ...
          {/* Right Column: Interactive Neural Ecosystem Visual & Staggered Node Grid */}
          <motion.div
            variants={shouldReduceMotion ? undefined : ecosystemContainerVariants}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.2 }}
          ...
          >
            <div className="relative w-full max-w-[506px] aspect-square flex items-center justify-center">
              ...
              {/* Center Node: YOUR BUSINESS with living adaptive core variants */}
              <motion.div
                variants={shouldReduceMotion ? ecosystemNodeVariants : livingNexusVariants}
                initial="hidden"
                animate={shouldReduceMotion ? 'visible' : ['visible', 'living']}
                className="relative z-20 flex flex-col items-center justify-center w-28 h-28 rounded-full bg-gradient-to-b from-[#181822] to-[#0d0d12] border-2 border-violet-500/60 shadow-[0_0_40px_rgba(139,92,246,0.35)] cursor-pointer group"
              >
              ...
              {/* Orbiting Nodes in Ecosystem Grid with living adaptive ecosystem variants */}
              {nodes.map((node) => {
                const rad = (node.angle * Math.PI) / 180;
                ...
                return (
                  <motion.div
                    ...
                    custom={{
                      ...
                      pathOffset: node.pathOffset,
                      xOffset: node.xOffset,
                      yOffset: node.yOffset,
                      scaleShift: node.scaleShift,
                      rotateShift: node.rotateShift,
                    }}
                    variants={shouldReduceMotion ? ecosystemNodeVariants : livingOrbitVariants}
                    initial="hidden"
                    animate={shouldReduceMotion ? 'visible' : ['visible', 'living']}
                  ...
                  >
                    <motion.button
                      ...
                      className={`flex flex-col items-center justify-center p-2 rounded-xl transition-all duration-300 group focus:outline-none ${
                        isSelected
                          ? 'bg-[#151520] border-2 border-violet-400 shadow-[0_0_25px_rgba(139,92,246,0.5)] scale-105'
                          : 'bg-[#0d0d12]/90 border border-white/[0.12] hover:border-violet-500/40 hover:bg-[#14141c]'
                      }`}
                    >
                      <div
                        className="w-7 h-7 rounded-lg flex items-center justify-center mb-1 transition-transform group-hover:scale-110"
                        style={{
            ...
            <motion.div
              variants={shouldReduceMotion ? undefined : telemetryCardVariants}
              className="w-full max-w-[420px] mt-4 p-4 rounded-xl bg-[#0c0c10]/90 border border-white/[0.1] backdrop-blur-md shadow-2xl transition-all duration-300"
            >
